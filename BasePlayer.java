...
public abstract class BasePlayer implements Player {
    ...
    private void setUsername(String username) {
        if (username == null || username.trim().isEmpty()) {
            throw new IllegalArgumentException(PLAYER_USER_NAME_NULL_OR_EMPTY);
        }
    ...
    public void setHealth(int healthPoints) {
        if (healthPoints < HEALTH_POINTS_MIN) {
            throw new IllegalArgumentException(PLAYER_HEALTH_COULD_NOT_BE_LESS_THAN_ZERO);
        }
    }
    ...
    public void takeDamage(int damagePoints) {
        if (damagePoints < DAMAGE_POINTS_MIN) {
            throw new IllegalArgumentException(DAMAGE_POINTS_PLAYER_LESS_THAN_ZERO);
        }
